...
{
	ESP_LOGD(TAG, "Reset slave using GPIO[%u]", H_GPIO_PIN_RESET_Pin);
...
static void transport_driver_event_handler(uint8_t event)
{
	switch(event)
	{
		case TRANSPORT_TX_ACTIVE:
		{
			...
			ESP_LOGD(TAG, "Base transport is set-up");
...
esp_err_t transport_drv_reconfigure(void)
{
	...
	ESP_LOGD(TAG, "Attempt connection with slave: retry[%u]", retry_slave_connection);
...
{
	ESP_LOGD(TAG, "capabilities: 0x%x",cap);
}
...
static void process_event(uint8_t *evt_buf, uint16_t len)
{
	...
	if (event->event_type == ESP_PRIV_EVENT_INIT) {
		...
		ESP_LOGD(TAG, "Received INIT event from ESP32 peripheral");
...
#define LENGTH_1_BYTE 1
	...
	/* TLV - Board type */
	ESP_LOGD(TAG, "Slave chip Id[%x]", ESP_PRIV_FIRMWARE_CHIP_ID);
...
int process_init_event(uint8_t *evt_buf, uint16_t len)
{
	...
	if ((chip_type != ESP_PRIV_FIRMWARE_CHIP_ESP32) &&
		...
		ESP_LOGE(TAG, "ESP board type is not mentioned, ignoring [%d]", chip_type);
		chip_type = ESP_PRIV_FIRMWARE_CHIP_UNRECOGNIZED;
		return -1;
	} else {
		ESP_LOGD(TAG, "ESP board type is : %d", chip_type);
		ESP_LOGI(TAG, "Chip is: %s%s%s%s%s%s",
